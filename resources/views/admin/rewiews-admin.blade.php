...
@section("styles")
   <link rel="stylesheet" href="{{ asset("./css/rewiew.css") }}">
@endsection
@section("description")
...
@section("content")
<div class="tool">
        <form method="post" action="{{ route("deleteRewiew") }}">
            @csrf
            <input type="number" name="rewiewId" class="rewiewId" hidden>
            <button>Удалить</button>
        </form>
   </div>
   <section class="rewiews-section">
        <div class="filter">
            <form action="{{ route("rewiews-admin") }}">
        ...
        @foreach ($rewiews as $rewiew)
            <div class="rewiew-card">
               ...
               <img src="{{ "../".$rewiew->avatar }}">
                ...
                <input type="number" value="{{ $rewiew->id }}" hidden>
            </div>
        @endforeach
        </div>
...
@endsection
@section("scripts")
    <script src="{{ asset("js/admin/rewiews.js") }}"></script>
